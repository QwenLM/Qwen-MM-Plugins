...
do_install() {
  ...
  # proxy capability: rewrite harness base_urls to point at local proxy (first-hop rule §4.4)
  case " $SELECTED_PLUGINS " in *" proxy "*|*" qwen-mm-plugins-proxy "*)
    proxy_rewrite_cc; proxy_rewrite_codex; proxy_rewrite_qwen_code ;;
  esac
...
do_uninstall() {
  ...
  # proxy capability: restore original harness base_urls after uninstalling proxy
  case " $removed " in *" proxy "*|*" qwen-mm-plugins-proxy "*)
    proxy_restore_base_urls ;;
  esac
