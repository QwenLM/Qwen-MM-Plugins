...
class VerifyGroundingArgs(BaseModel):
    image: str = Field(description="Path to the frame image.")
    bbox: list[float] = Field(description="[x1,y1,x2,y2] region to verify (0-1000 normalized or pixels).")
    label: str = Field(description="The label the region is claimed to show (e.g. 'chair').")
...
TOOL: dict[str, Any] = {
    ...
    "description": (
        "Independently VERIFY a grounding: ask the VLM whether the region in `bbox` really shows "
        "`label`. Run before finalizing any distance/relation conclusion — a wrong bbox is the #1 "
        "cause of a flipped spatial answer. Returns a match verdict + confidence."
    ),
    "args": VerifyGroundingArgs,
}
