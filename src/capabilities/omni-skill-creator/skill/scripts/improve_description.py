...
by calling an LLM via an OpenAI-compatible API (DashScope, etc.).
"""
...
def _load_parse_skill_md():
    ...
    spec = importlib.util.spec_from_file_location("_omni_skill_creator_improve_description_utils", utils_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load description utilities from {utils_path}")
...
def _call_llm(
    ...
    """Call an LLM via OpenAI-compatible API and return the text response."""
    from openai import OpenAI

    base, key = resolve_openai_endpoint({"base_url": api_base, "api_key": api_key})
...
def improve_description(
    ...
    prompt = f"""You are optimizing the description of an Agent Skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the agent sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_llm(prompt, model, api_base, api_key)
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_llm(shorten_prompt, model, api_base, api_key)
...
def main():
    ...
    parser.add_argument("--api-base", default=None, help="API base URL (default: DashScope)")
    parser.add_argument(
