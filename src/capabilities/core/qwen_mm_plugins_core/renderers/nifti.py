...
def _resolve_slice_indices(
    ...
    requested_indices = opts.get("nifti_slice_indices")
    requested_positions = opts.get("nifti_slice_positions")
    if requested_indices is not None and requested_positions is not None:
        raise ValueError("nifti_slice_indices and nifti_slice_positions are mutually exclusive")

    if requested_indices is not None:
        if not isinstance(requested_indices, (list, tuple)) or not requested_indices:
            raise ValueError("nifti_slice_indices must be a non-empty list")
        if len(requested_indices) > MAX_SLICES:
            raise ValueError(f"nifti_slice_indices accepts at most {MAX_SLICES} values")
        indices: list[int] = []
        for value in requested_indices:
            if isinstance(value, bool) or not isinstance(value, (int, np.integer)):
                raise ValueError("nifti_slice_indices must contain only integers")
            index = int(value)
    ...
    if requested_positions is not None:
        if not isinstance(requested_positions, (list, tuple)) or not requested_positions:
            raise ValueError("nifti_slice_positions must be a non-empty list")
        if len(requested_positions) > MAX_SLICES:
            raise ValueError(f"nifti_slice_positions accepts at most {MAX_SLICES} values")
        positions: list[float] = []
        for value in requested_positions:
            if isinstance(value, bool) or not isinstance(value, (int, float, np.integer, np.floating)):
                raise ValueError("nifti_slice_positions must contain only numbers")
            position = float(value)
            if not math.isfinite(position) or not 0.0 <= position <= 1.0:
                raise ValueError("nifti_slice_positions values must be finite and between 0 and 1")
    ...
    count = opts.get("nifti_num_slices", DEFAULT_NUM_SLICES)
    if isinstance(count, bool) or not isinstance(count, (int, np.integer)):
        raise ValueError("nifti_num_slices must be an integer")
    count = int(count)
    if not 1 <= count <= MAX_SLICES:
        raise ValueError(f"nifti_num_slices must be between 1 and {MAX_SLICES}")
...
def _parse_volume_pages(pages: str, total_volumes: int) -> list[int]:
    """Parse a strict 1-based 4D volume range into unique 0-based indices."""
    if not isinstance(pages, str) or not pages.strip():
        raise ValueError("NIfTI volume pages must be a non-empty 1-based range")

    result: list[int] = []
    for raw_part in pages.split(","):
        part = raw_part.strip()
        if not part:
            raise ValueError(f"Invalid NIfTI volume pages range: {pages!r}")
        try:
            if "-" in part:
                if part.count("-") != 1:
                    raise ValueError
            ...
            raise ValueError(
                f"Invalid NIfTI volume pages range {pages!r}; use values such as '1', '1-3', or '1,3'"
            ) from None

        for page in requested:
            if not 1 <= page <= total_volumes:
                raise ValueError(f"NIfTI volume page {page} is outside 1..{total_volumes}")
...
def _resolve_intensity(
    ...
    mode = opts.get("nifti_intensity_mode", "auto_volume")
    if mode not in ("auto_volume", "window", "preset"):
        raise ValueError("nifti_intensity_mode must be 'auto_volume', 'window', or 'preset'")

    config: dict[str, Any] = {"mode": mode, **statistics}
    if mode == "auto_volume":
        if opts.get("nifti_window_center") is not None or opts.get("nifti_window_width") is not None:
            raise ValueError("nifti_window_center/width require nifti_intensity_mode='window'")
        if opts.get("nifti_window_preset") is not None:
            raise ValueError("nifti_window_preset requires nifti_intensity_mode='preset'")
        percentile_low = _finite_number(
            "nifti_percentile_low",
            opts.get("nifti_percentile_low", DEFAULT_PERCENTILE_LOW),
        )
        percentile_high = _finite_number(
            "nifti_percentile_high",
            opts.get("nifti_percentile_high", DEFAULT_PERCENTILE_HIGH),
        )
    ...
    if mode == "window":
        if opts.get("nifti_window_preset") is not None:
            raise ValueError("nifti_window_preset cannot be combined with nifti_intensity_mode='window'")
        center_value = opts.get("nifti_window_center")
        width_value = opts.get("nifti_window_width")
        if center_value is None or width_value is None:
            raise ValueError("nifti_intensity_mode='window' requires both nifti_window_center and nifti_window_width")
        center = _finite_number("nifti_window_center", center_value)
        width = _finite_number("nifti_window_width", width_value)
        if width <= 0:
            raise ValueError("nifti_window_width must be greater than 0")
        config.update(
            {
                "center": center,
                "width": width,
    ...
    preset = opts.get("nifti_window_preset")
    if preset not in WINDOW_PRESETS:
        supported = ", ".join(WINDOW_PRESETS)
        raise ValueError(f"nifti_intensity_mode='preset' requires nifti_window_preset to be one of: {supported}")
    if opts.get("nifti_window_center") is not None or opts.get("nifti_window_width") is not None:
        raise ValueError("nifti_window_center/width cannot be combined with nifti_intensity_mode='preset'")
...
def _metadata_text(
    ...
    if volume_indices:
        ...
        if len(volume_indices) == 1:
            lines.append(f"- Selected 3D volume: page {pages[0]} / index {indices_4d[0]} (of {shape[3]})")
        else:
            lines.append(f"- Selected 3D volumes: pages {pages}; indices {indices_4d} (of {shape[3]})")
        if defaulted_4d:
        ...
        if volume_selection_truncated:
            lines.append("- Volume selection was truncated by max_pages")
    lines.extend(
        (
            (
...
def render(path: str, **opts: Any) -> list[dict[str, Any]]:
    ...
    source_axis_value = opts.get("nifti_slice_axis", DEFAULT_SLICE_AXIS)
    if isinstance(source_axis_value, bool) or not isinstance(source_axis_value, (int, np.integer)):
        raise ValueError("nifti_slice_axis must be 0, 1, or 2")
    ...
    defaulted_4d = source.ndim == 4 and not pages
    volume_selection_truncated = False
    if source.ndim == 4:
        max_pages = opts.get("max_pages", DEFAULT_MAX_PAGES)
        if isinstance(max_pages, bool) or not isinstance(max_pages, (int, np.integer)):
            raise ValueError("max_pages must be a positive integer")
        max_pages = int(max_pages)
        if max_pages < 1:
            raise ValueError("max_pages must be a positive integer")
        requested_volumes = _parse_volume_pages(pages, int(source.shape[3])) if pages else [0]
        volume_selection_truncated = len(requested_volumes) > max_pages
        volume_indices: list[int | None] = requested_volumes[:max_pages]
    else:
        volume_indices = [None]
    ...
    for volume_index in volume_indices:
        ...
        for index in slice_indices:
            ...
            if rendered_images and response_bytes + candidate_bytes > MAX_RESPONSE_BYTES:
                ...
                stopped_at = (
                    f"volume page {volume_index + 1}, source index {index}"
                    if volume_index is not None
